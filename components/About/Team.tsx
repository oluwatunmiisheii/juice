...
export const Team = () => {
  return (
    <Section title="Our Team" description="Lorem ipsum dolor sit amet, consectetur adipiscing elit.">
      {teams.map((team) => (
        <div key={spacesToUnderScore(team.title)} className="mt-10" data-aos="fade-up">
          <h3 className="text-[28px] leading-[42px] font-bold mb-4">{team.title}</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {team.members.map((member) => (
              <div
                key={spacesToUnderScore(member.image)}
                className="bg-white rounded-xl backdrop-blur-[20px]"
                data-aos="fade-up"
              >
